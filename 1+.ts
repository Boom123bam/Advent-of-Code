...
function concatFirstLast(line: string) {
  ...
  if (!first || !last) throw Error("bad line: " + line);
  return Number(first + last);
}

function addDigits(line: string) {
  let res = line;
  nums.forEach((num, index) => {
    res = res.replaceAll(num, num + (index + 1).toString() + num);
  });
  return res;
}
...
const sum = lines.reduce(
  (prev, line) => prev + concatFirstLast(addDigits(line)),
  0
);
