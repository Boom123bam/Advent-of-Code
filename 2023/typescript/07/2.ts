let input = await Deno.readTextFile("07/input.txt");
...
function getHandTypeScore(hand: string) {
  ...
  if (cardsCount.length) cardsCount[cardsCount.length - 1] += numOfJs;
  else cardsCount.push(numOfJs);
...
// console.log(getHandTypeScore("JJJJJ"));
